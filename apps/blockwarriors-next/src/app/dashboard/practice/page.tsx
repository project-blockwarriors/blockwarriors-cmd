...
export default function PracticePage() {
  ...
  // Check if tokens are available (generated by Minecraft server)
  const hasTokens =
    matchData?.tokens &&
    (matchData.tokens.redTeam.length > 0 ||
      matchData.tokens.blueTeam.length > 0);

  // Check if match is waiting for tokens to be generated
  const waitingForTokens =
    matchId && matchData?.match_status === 'Queuing' && !hasTokens;

  // Check if all players are logged in (all tokens used)
